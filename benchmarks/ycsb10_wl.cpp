...
#include "ycsb10_store_procedure.h"

#if WORKLOAD == YCSB10

int WorkloadYCSB10::next_tid;

RC WorkloadYCSB10::init() {
    workload::init();
    next_tid = 0;
...
RC WorkloadYCSB10::init_schema(string schema_file) {
...
int
WorkloadYCSB10::key_to_part(uint64_t key) {
    return 0;
}

uint32_t
WorkloadYCSB10::key_to_node(uint64_t key, uint32_t table_id)
{
    return key % g_num_nodes;
}

// init table in parallel
void WorkloadYCSB10::init_table_parallel() {
...
void * WorkloadYCSB10::init_table_slice() {
...
StoreProcedure *
WorkloadYCSB10::create_store_procedure(TxnManager * txn, QueryBase * query)
{
    return new YCSB10StoreProcedure(txn, query);
}

QueryBase *
WorkloadYCSB10::gen_query()
{
    QueryBase * query = (QueryYCSB10 *) MALLOC(sizeof(QueryYCSB10));
    new(query) QueryYCSB10();
    return query;
}

QueryBase *
WorkloadYCSB10::clone_query(QueryBase * query)
{
    QueryYCSB10 * q = (QueryYCSB10 *) query;
    QueryYCSB10 * new_q = new QueryYCSB10(q->get_requests(), q->get_request_count());
    return new_q;
}

QueryBase *
WorkloadYCSB10::deserialize_subquery(char * data)
{
    QueryYCSB10 * query = (QueryYCSB10 *) MALLOC(sizeof(QueryYCSB10));
    new(query) QueryYCSB10(data);
    return query;
}

void
WorkloadYCSB10::table_to_indexes(uint32_t table_id, set<INDEX *> * indexes)
{
    assert(table_id == 0);
...
uint64_t
WorkloadYCSB10::get_primary_key(row_t * row)
{
    uint64_t key;
    row->get_value(0, &key);
    return key;
