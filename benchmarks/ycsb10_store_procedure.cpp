...
YCSB10StoreProcedure::YCSB10StoreProcedure(TxnManager * txn_man, QueryBase * query)
    : StoreProcedure(txn_man, query)
{
}

YCSB10StoreProcedure::~YCSB10StoreProcedure()
{
}

RC
YCSB10StoreProcedure::execute()
{
    RC rc = RCOK;
    WorkloadYCSB10 * wl = (WorkloadYCSB10 *) GET_WORKLOAD;
    INDEX * index = wl->the_index;
    QueryYCSB10 * query = (QueryYCSB10 *) _query;
    RequestYCSB10 * requests = query->get_requests();
    assert(_query);
#if SINGLE_PART_ONLY
    for ( ; _curr_query_id < query->get_request_count(); _curr_query_id ++) {
        RequestYCSB10 * req = &requests[ _curr_query_id ];
...
void
YCSB10StoreProcedure::txn_abort()
{
